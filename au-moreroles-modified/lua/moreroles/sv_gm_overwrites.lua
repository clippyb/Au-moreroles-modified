...
function GAMEMODE:Game_CheckWin(reason)
  ...
  for team, num in pairs(teamsAlive) do
    -- team wins if it has half or more of the living players, but not if the Glitch is alive and at least one Imposter is alive
    if num >= totalPlayers / 2 and not (aliveGlitch and aliveImposters) then
      -- if team has exactly half of the players they only won if no other player can kill them
      if num == totalPlayers / 2 then
        local canBeKilled = false
